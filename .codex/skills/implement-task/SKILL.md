---
name: implement-task
description: Implement one scoped GIS platform task from a spec or task index. Use when the user says `$implement-task`, asks to implement a track-scoped task such as `backend TASK-01`, `frontend TASK-04`, or `infra TASK-01`, or wants Codex to execute a defined task while staying inside acceptance criteria, running verification, and updating task status notes.
---
...
2. Identify the requested task.
   - Prefer task files under `docs/tasks/`, for example `docs/tasks/backend/03-location-crud.md`.
...
```text
Use $implement-task to implement backend TASK-03 from docs/tasks/backend/03-location-crud.md.
```
